<?php
/**
 * ARQUIVO: PERFIL DO USUÁRIO
 * Papel: Escritório Pessoal e Caixa de Notificações
 */
...
// CAIXA DE ADVERTÊNCIAS (LIVROS CONFISCADOS)
...
// MINHA PRATELEIRA PARTICULAR (LIVROS ATIVOS)
...
<body class="<?php echo $tema; ?>">
    ...
    <main class="content">
        ...
        <?php while ($lv = $livros_excluidos->fetch_assoc()): ?>
            <div class="alerta-exclusao" style="position: relative; background:#F15256; color:white; padding:14px 40px 14px 20px; border-radius:12px; margin-bottom:12px; font-weight:500;">
                ...
                <!-- BOTÃO "AMASSAR PAPEL DE AVISO" -->
                <a href="limpar_alerta.php?id=<?php echo $lv['id']; ?>" style="position: absolute; top: 50%; right: 15px; transform: translateY(-50%); background: none; border: none; color: white; font-size: 1.2rem; cursor: pointer; opacity: 0.8; text-decoration: none;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.8'">
        ...
        <!-- PRATELEIRA PARTICULAR -->
        <section class="books-section">
            <h2>Meus Livros</h2>
            <div class="livros-grid">
                <?php if ($meus_livros->num_rows > 0): ?>
                    <!-- O FUNCIONÁRIO PARTICULAR -->
    ...
    <script>
        // MECANISMO DE LUZ DO CLIENTE (JavaScript)
